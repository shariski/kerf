...
- `/welcome` — expanded from a 60-word private-beta splash into a 6-section landing page (~280 lines, ~750 words) targeting struggling split-keyboard owners. Sections: hero, why split is hard, how the engine adapts, supported keyboards (Sofle, Lily58), session mockup, final CTA. Drops the "Currently in private beta" gate framing — primary CTA is now "Start practicing →". JSON-LD `SoftwareApplication` schema added for Google rich-result eligibility. Render test added at `src/routes/welcome.test.tsx`.
- **Biome `noExplicitAny` is now also disabled in test files** alongside `noNonNullAssertion` — same `biome.json` override block. Test mocks legitimately use `any` for runtime patching of error objects (`(err as any).isRedirect = true`) and to bypass strict typing on third-party mock returns (`mockResolvedValue(mockSession as any)`); these are local-to-test patterns with no production code path. Net: 4 warnings → 0.
- **Refactored `domain/dashboard/temporalPatterns.ts` hour/day-bucket builders** to use `?? 0` defaults instead of `!` non-null assertions. The dense fixed-size arrays (24-hour, 7-day) are guaranteed populated by the `Array.fill(0)` initialization, but TypeScript's `noUncheckedIndexedAccess` can't see that. Lifting the bucket count/sum into local `const`s reads better and removes 8 `!` operators. All 19 existing tests pass unchanged.
...
- **Two pre-existing test failures unrelated to features.** (1) `src/server/profile.test.ts`'s "accepts a well-formed input" case asserted `toEqual(input)` on `validateCreateProfileInput`, but the function always returns `nickname` (normalized to `null` when absent) — test was written before the `nickname` field existed. Now asserts `{ ...input, nickname: null }`. (2) `vitest.config.ts` did not exclude `tests/**`, so vitest was loading `tests/a11y.spec.ts` (a Playwright spec) and failing with `test() not expected to be called here`. Added `tests/**` to the exclude list; Playwright remains the only runner for that directory.
- **Pre-session CTA felt unresponsive after refresh / cold load.** The "Continue adaptive practice" button (and Enter shortcut) on `/practice` silently no-op'd while the 1.6 MB `/corpus.json` was still loading, because `generateSessionAndShowBriefing` early-returned on `corpus.status !== "ready"` with no UI feedback. Two-part fix: (1) `PreSessionStage` accepts an `awaitingCorpus` prop that surfaces a `loading word list…` meta line + `aria-busy` on the CTA, and the parent route queues the user's start intent and auto-fires the briefing once corpus arrives — extra clicks while waiting collapse into one queued start instead of being dropped; (2) `/corpus.json` now ships with `Cache-Control: public, max-age=31536000, immutable` from the prod entrypoint and is fetched as `/corpus.json?v=${CORPUS_VERSION}` so a version bump cleanly invalidates the cache. Refreshes after the first cold load are now zero-network for the corpus. (`src/components/practice/PreSessionStage.tsx`, `src/routes/practice.tsx`, `src/domain/corpus/loader.ts`, `scripts/serve.mjs`)
...
- **Dashboard, nav, and practice-pill a11y semantics** — eliminates 12 a11y errors across 10 files via the same "let HTML carry the meaning instead of ARIA" principle as the post-session fix:
  ...
  - `PhaseBadge.tsx`, `LiveWpm.tsx`, `KeyboardContextPill.tsx`: removed redundant `aria-label`s that duplicated visible text content.
- **Post-session a11y semantics** — replaced fake list scaffolding (`role="list"` / `role="listitem"` on `<div>`s) with real `<ul>` / `<li>` in both `PostSessionStage.tsx` and `DrillPostSessionStage.tsx`. Replaced focusable `<span>` (`tabIndex={0}` + `aria-label`) used for error chars with `<button type="button">`, which natively supports both. Dropped redundant `aria-label`s on stat-value `<div>`s — the visible label/value/delta structure already conveys the same content to screen readers without overriding it. Eliminates 18 Biome a11y errors (45% of total). (`PostSessionStage.tsx`, `DrillPostSessionStage.tsx`, `styles.css`)
