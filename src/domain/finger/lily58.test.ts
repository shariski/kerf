...
describe("LILY58_BASE_LAYER", () => {
  describe("coverage", () => {
    it("assigns every lowercase letter a–z", () => {
      ...
      for (let code = 0x61; code <= 0x7a; code++) {
        ...
        if (!LILY58_BASE_LAYER[ch]) missing.push(ch);
      }
    ...
    it("assigns space, comma, period, semicolon, apostrophe, slash, and hyphen", () => {
      for (const ch of [" ", ",", ".", ";", "'", "/", "-"]) {
        expect(LILY58_BASE_LAYER[ch]).toBeDefined();
      }
    });
  });
  ...
  describe("invariants", () => {
    it("uses only valid hand + finger values and non-negative row/col", () => {
      for (const [ch, a] of Object.entries(LILY58_BASE_LAYER)) {
  ...
  describe("home row anchors", () => {
    ...
    it("';' is right pinky home row", () => {
      expect(LILY58_BASE_LAYER[";"]).toMatchObject({
        hand: "right",
        finger: "pinky",
        row: 2,
      });
