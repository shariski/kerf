      ...
      <div className="kerf-app-footer-row">
        {LINKS.map((link, i) => (
          <span key={link.to} className="kerf-app-footer-cell">
            {link.kind === "internal" ? (
              // biome-ignore lint/suspicious/noExplicitAny: /contact route registered in Task 2; cast avoids premature route-registration dependency
              <Link to={link.to as any} className="kerf-app-footer-link">
                {link.label}
              </Link>
            ) : (
              <a
